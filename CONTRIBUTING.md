...
make test            # this repo's own suite: name resolution + the version floors
make packages-check  # do all install/packages.txt names resolve and clear their floors?
...
(`help` is the seventh). The list is declared once, in `dotfiles-core`'s
`scripts/make-vocabulary.txt`, so a target means the same thing in every repo in the
fleet — see [dotgibson/dotfiles-core#691](https://github.com/dotgibson/dotfiles-core/issues/691).
