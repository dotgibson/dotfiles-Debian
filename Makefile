...
check: lint ## lint + a hermetic --links-only run against a throwaway HOME
	...
	@# under $$tmp. Reproduced against the byte-identical recipe in dotfiles-Fedora, on a
	@# Fedora 44 box (dotgibson/dotfiles-Fedora#153): the whole tree — zsh/, nvim,
	@# starship.toml, tmux, git, mise, lazygit, atuin, jj, sesh, tealdeer — landed in the
	@# exported XDG_CONFIG_HOME while the target reported MISSING symlinks on a perfectly
	@# good checkout. Only the package manager differs between the two repos here; the
	@# --links-only path and the Core lib it calls are the same code. `env -u` for the five
	@# variables the bootstrap path actually consults is the fix, and dotfiles-openSUSE had
	@# already reached it locally.
	@#
